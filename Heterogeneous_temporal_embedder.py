...
from GNN_temporal import GNNEncoder, Classifier_heterogeneous
...
def get_features(data, degree):
    ...
    return student_id, student_feat
...
class EmbedderHeterogeneous(torch.nn.Module):
    def __init__(
        ...
        self.student_lin = torch.nn.Linear((student_inchannel-1)*self.degree, hidden_channels[0]) if student_inchannel is not None else None
    ...
    def forward(self, data):
        ...
        if self.item_lin is not None:
            ...
            self.item_x = self.item_emb(student_id)
    ...
    def get_embeddings(self, data):
        ...
        if self.student_lin is not None:
            student_x = self.student_lin(student_feat) + self.student_emb(student_id)
        else:
            student_x = self.student_emb(student_id)
